 ...
 *   v0.12 - v0.11 only reset pingConsecutive inside checkPing() itself,
 *           so it still ran unbounded whenever Power, Button, or the
 *           physical switch changed the actor — those call commandActor()
 *           too but never touched the counter. A counter that had grown
 *           large while the raw ping result stayed constant for a long
 *           time (host online for hours, or offline for hours) would
 *           therefore satisfy confirmCountOn/Off on the very next tick
 *           after such a trigger fired, instantly reverting it: e.g.
 *           Power turns the actor OFF at shutdown, but the NIC still
 *           answers ping briefly on standby power (WoL nachlauf) —
 *           with the old, still-huge "online" counter this alone was
 *           enough to trigger an immediate Ping ON a moment later,
 *           which Ping then reverted again once the NIC actually went
 *           quiet (OFF → ON → OFF). Symmetrically at power-on, Power
 *           fires ON early during POST while the OS/network is not up
 *           yet, but the still-huge "offline" counter from the hours
 *           the host was off let Ping immediately fire OFF again
 *           (ON → OFF → ON once boot finished). Fix: pingConsecutive
 *           is now reset inside commandActor() itself, so ANY trigger
 *           source invalidates the streak and Ping always needs fresh
 *           confirmCountOn/Off matching results before it can act
 *           again, regardless of who changed the actor last.
 * =====================================================================
 */
...
function commandActor(dev, value) {
    ...
    // Any state change, from ANY trigger source, invalidates whatever
    // consecutive-ping streak had already accumulated. Without this,
    // a counter that grew large while the raw ping result was stable
    // for a long time (host online for hours, or offline for hours)
    // could satisfy confirmCountOn/Off on the very next tick after
    // Power/Button/physical-switch just changed the actor, instantly
    // overriding that change instead of requiring fresh confirmations.
...
function checkPing(dev) {
    ...
    exec(cmd, (error) => {
        ...
        if (isOnline && dev.ping.on && !currentState) {
            ...
            commandActor(dev, true); // resets pingConsecutive internally
            ...
            const confirmedCount = st.pingConsecutive;
            commandActor(dev, false); // resets pingConsecutive internally
