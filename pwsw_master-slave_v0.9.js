...
function checkPing(dev) {
    ...
    const st = deviceState.get(dev.name);
    const cmd = `ping -c 1 -W ${dev.ping.timeoutSec} ${dev.ping.host}`;
    exec(cmd, (error) => {
        ...
        if (isOnline && dev.ping.on && !currentState) {
            commandActor(dev, true);
            log(`✅ ${dev.name}-Slave ON (Ping: ${dev.ping.host} online, confirmed ${st.pingConsecutive}x)`, "info");
        } else if (!isOnline && dev.ping.off && currentState) {
            commandActor(dev, false);
            log(`⛔ ${dev.name}-Slave OFF (Ping: ${dev.ping.host} offline, confirmed ${st.pingConsecutive}x)`, "info");
        }
